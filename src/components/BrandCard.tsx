...
export function BrandCard({ brand }: { brand: BrandDealer }) {
  ...
  return (
    <Link
      to={`/brand/${brand.slug}`}
      className="group atlas-card flex flex-col text-right p-5 shadow-soft-md"
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex h-14 w-14 sm:h-16 sm:w-16 shrink-0 items-center justify-center rounded-2xl border border-border bg-surface p-2.5">
          {logo ? (
      ...
      <div className="mt-5">
        <div className="flex flex-wrap items-center gap-2">
          <h3 className="font-display text-xl sm:text-2xl font-semibold leading-tight text-foreground">{brand.brandName}</h3>
      ...
      <div className="mt-5 flex items-center justify-between border-t border-border pt-4">
        <span className="text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground group-hover:text-primary transition-colors">
          عرض الوكيل
        </span>
